...
const copy_to_host_scp = (done) => {

    const DEV_CONFIG = get_dev_config();
    const newest_build = fetch_latest_build();
    const command = [
        'scp',
        `-i ${DEV_CONFIG.KEY}`,
        `build/${newest_build}`,
...
const copy_to_host_http = (done) => {

    const DEV_CONFIG = get_dev_config();
    const newest_build = fetch_latest_build();
    console.log('Uploading ' + newest_build);
    const fstats = fs.statSync(`${process.cwd()}/build/${newest_build}`);

    const options = {
        hostname: DEV_CONFIG.HOST,
        auth: `${DEV_CONFIG.USER}:${DEV_CONFIG.PASS}`,
        path: `/mgmt/shared/file-transfer/uploads/${newest_build}`,
        method: 'POST',
    };
    ...
    const fstream = fs.createReadStream(`${process.cwd()}/build/${newest_build}`);
    fstream.on('end', () => {
        req.end();
    });
    fstream.pipe(req);
