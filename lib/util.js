...
//const strings, urls etc
const ICR_PACKAGE_MANAGEMENT_TASKS = '/mgmt/shared/iapp/package-management-tasks';
...
const buildRpm = (cwd, opts, done) => {
    ...
    const destDir = opts.destDir || './build';
...
const pollTaskStatus = (opts, link, cb) => {
    const options = prepareReqOptions(opts, link);
...
const installRpmOnBigIp = (opts, rpmpath, cb) => {
    ...
    const options = prepareReqOptions(opts, ICR_PACKAGE_MANAGEMENT_TASKS);
    options.method = 'POST';
...
const queryInstalledPackages = (opts, cb) => {
    const options = prepareReqOptions(opts, ICR_PACKAGE_MANAGEMENT_TASKS);
    options.method = 'POST';
    options.headers = Object.assign(opts.headers || {}, { 'Content-Type': 'application/json' });
...
const simpleHttpsGet = (opts, path, cb) => {
    const options = prepareReqOptions(opts, path);
    const req = https.request(options, (res) => {
        const error = checkForHttpError(res);
        if (error) {
            if (cb) cb(error);
