...
const pollRpmInstallStatus = (link, cb) => {
    ...
    const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
            ...
            if( status.status === 'STARTED' ) {
                setTimeout( () => { pollRpmInstallStatus(link, cb); }, 2000);
            } else {
...
exports.pollRpmInstallStatus = pollRpmInstallStatus;
...
const installRpmOnBigIp = (rpmpath, done) => {
    ...
    const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
            ...
            pollRpmInstallStatus(status_link, done);
        });
    });
